 ...
 */
export function GenerativeMountainScene({ isDarkMode }) {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        color: { value: new THREE.Color(initialColor) },
      },
      vertexShader: `
        uniform float time;
  ...
  useEffect(() => {
    if (materialRef.current) {
      const targetColor = isDarkMode ? "#7dd3fc" : "#22c55e"; // sky-300 in dark, green-500 in light
      materialRef.current.uniforms.color.value.set(targetColor);
    }
  }, [isDarkMode]);

  // Dark mode: slate-900, Light mode: pleasant white-blue sky
  const bgColor = isDarkMode ? '#0f172a' : '#e0f2fe';

  return <div ref={mountRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 0, backgroundColor: bgColor, transition: 'background-color 0.3s ease' }} />;
}
